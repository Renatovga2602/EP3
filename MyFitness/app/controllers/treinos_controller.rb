class TreinosController < ApplicationController
  ...
  @treinos = Treino.all
  if params[:search]
    @treinos = Treino.search(params[:search]).order("created_at DESC")
  else
    @treinos = Treino.all.order("created_at DESC")
  end
end
  ...
  private
    ...
    def treino_params
      params.require(:treino).permit(:inferior, :triceps, :abslombar, :dorsais, :deltoides, :biceps, :peitorais, :series, :repeticoes, :email)
    end
end
